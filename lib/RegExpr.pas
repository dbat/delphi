...

{
 ...
 5. RegExp Studio application and all the visual components as
    well as documentation is not part of the TRegEx library
...
// ======== Determine compiler
{$IFDEF VER80}Sorry, TRegEx is for 32 - bits Delphi only.Delphi 1 is not supported(and whos really care today?!).{$ENDIF}
...
// ======== Define options for TRegEx engine
...
// Asserts used to catch 'strange bugs' in TRegEx implementation (when something goes
...
type
  TRegExInvertCaseFunction = function(const Ch: REChar): REChar
    of object;
...
const
  ...
  RegExprModifierI: boolean = TRUE;
  RegExprModifierR: boolean = FALSE;
  RegExprModifierS: boolean = FALSE;
  RegExprModifierG: boolean = FALSE;
  ...
  RegExprLineSeparators: RegExprString = #$D#$A // default value for LineSeparators
{$IFDEF UniCode} + #$B#$C#$2028#$2029#$85{$ENDIF}; //###0.947
  RegExprLinePairedSeparator: RegExprString = #$D#$A; // default value for LinePairedSeparator
  { if You need Unix-styled line separators (only \n), then use:
  RegExprLineSeparators = #$A; RegExprLinePairedSeparator = '';
  }
...
const
  ...
  // Don't change it! It's defined by internal TRegEx design.
...
{$ENDIF}
  ...
  TinySetLen = 3;
  // if range includes more than TinySetLen chars, //###0.934
...
{$ENDIF}
  ...
  TRegExReplaceFunction = function(ARegExpr: TRegEx): string
    of object;
...
{$ENDIF}
    ...
    fInvertCase: TRegExInvertCaseFunction; //###0.927
...
{$ENDIF}
  ...
  public
    ...
    // Regular expression.
    // For optimization, TRegEx will automatically compiles it into 'P-code'
...
{$ENDIF}
    ...
    function Replace(AInputStr: RegExprString;
      AReplaceFunc: TRegExReplaceFunction)
...
{$ENDIF}
    ...
    property InvertCase: TRegExInvertCaseFunction read fInvertCase write fInvertCase; //##0.935
...
const
  RegExprInvertCaseFunction: TRegExInvertCaseFunction = {$IFDEF FPC}nil{$ELSE}TRegEx.InvertCaseFunction{$ENDIF};
...
procedure SpliTRegEx(const ARegExpr, AInputStr: RegExprString; APieces: TStrings);
...
const
  TRegExVersionMajor: integer = 0;
  TRegExVersionMinor: integer = 952;
  // TRegEx.VersionMajor/Minor return values of this constants
...
begin
  ...
  for i := 1 to Len do Dest[i - 1] := Source[i];
  Dest[Len] := #0;
  Result := Dest;
...
var
  i: Integer;
begin
  for i := 0 to MaxLen - 1 do Dest[i] := Source[i];
  Result := Dest;
...
begin
  S1 := Str1;
  S2 := Str2;
  if Copy(S1, 1, MaxLen) > Copy(S2, 1, MaxLen) then Result := 1
  else
    if Copy(S1, 1, MaxLen) < Copy(S2, 1, MaxLen) then Result := -1
    else Result := 0;
      ...
      case ARegExpr[i] of
        '(': begin
            if (i < Len) and (ARegExpr[i + 1] = '?') then begin
              ...
              if i > Len then Result := -1 // unbalansed '('
              else if TRegEx.ParseModifiersStr(System.Copy(ARegExpr, i, i - i0), Modif) then AExtendedSyntax := (Modif and MaskModX) <> 0;
            end
        ...
        ')': begin
            if StackIdx = 0 then Result := i // unbalanced ')'
            else begin
              dec(StackIdx);
              with Stack[StackIdx] do begin
        ...
        '[': begin
            ...
            i0 := i;
            inc(i);
            if ARegExpr[i] = ']' then inc(i); // cannot be 'emty' ranges - this interpretes
            // as ']' by itself
            while (i <= Len) and (ARegExpr[i] <> ']') do
              if ARegExpr[i] = EscChar {//###0.942} then inc(i, 2) // skip 'escaped' char to prevent stopping at '\]'
              else inc(i);
            if (i > Len) or (ARegExpr[i] <> ']') {//###0.942} then Result := -(i0 + 1); // unbalansed '[' //###0.942
          end;
        '#': if AExtendedSyntax then begin
            // skip eXtended comments
            while (i <= Len) and (ARegExpr[i] <> #$D) and (ARegExpr[i] <> #$A) do inc(i);
            // do not use [#$d, #$a] due to UniCode compatibility
            while (i + 1 <= Len) and ((ARegExpr[i + 1] = #$D) or (ARegExpr[i + 1] = #$A)) do inc(i);
            // attempt to work with different kinds of line separators
...
function TRegEx.ErrorMsg(AErrorID: integer): RegExprString;
begin
  case AErrorID of
    reeOk: Result := 'No errors';
    reeCompNullArgument: Result := 'TRegEx(comp): Null Argument';
    reeCompRegexpTooBig: Result := 'TRegEx(comp): Regexp Too Big';
    reeCompParseRegTooManyBrackets: Result := 'TRegEx(comp): ParseReg Too Many ()';
    reeCompParseRegUnmatchedBrackets: Result := 'TRegEx(comp): ParseReg Unmatched ()';
    reeCompParseRegUnmatchedBrackets2: Result := 'TRegEx(comp): ParseReg Unmatched ()';
    reeCompParseRegJunkOnEnd: Result := 'TRegEx(comp): ParseReg Junk On End';
    reePlusStarOperandCouldBeEmpty: Result := 'TRegEx(comp): *+ Operand Could Be Empty';
    reeNestedSQP: Result := 'TRegEx(comp): Nested *?+';
    reeBadHexDigit: Result := 'TRegEx(comp): Bad Hex Digit';
    reeInvalidRange: Result := 'TRegEx(comp): Invalid [] Range';
    reeParseAtomTrailingBackSlash: Result := 'TRegEx(comp): Parse Atom Trailing \';
    reeNoHexCodeAfterBSlashX: Result := 'TRegEx(comp): No Hex Code After \x';
    reeHexCodeAfterBSlashXTooBig: Result := 'TRegEx(comp): Hex Code After \x Is Too Big';
    reeUnmatchedSqBrackets: Result := 'TRegEx(comp): Unmatched []';
    reeInternalUrp: Result := 'TRegEx(comp): Internal Urp';
    reeQPSBFollowsNothing: Result := 'TRegEx(comp): ?+*{ Follows Nothing';
    reeTrailingBackSlash: Result := 'TRegEx(comp): Trailing \';
    reeRarseAtomInternalDisaster: Result := 'TRegEx(comp): RarseAtom Internal Disaster';
    reeBRACESArgTooBig: Result := 'TRegEx(comp): BRACES Argument Too Big';
    reeBracesMinParamGreaterMax: Result := 'TRegEx(comp): BRACE Min Param Greater then Max';
    reeUnclosedComment: Result := 'TRegEx(comp): Unclosed (?#Comment)';
    reeComplexBracesNotImplemented: Result := 'TRegEx(comp): If you want take part in beta-testing BRACES ''{min,max}'' and non-greedy ops ''*?'', ''+?'', ''??'' for complex cases - remove ''.'' from {.$DEFINE ComplexBraces}';
    reeUrecognizedModifier: Result := 'TRegEx(comp): Urecognized Modifier';
    reeBadLinePairedSeparator: Result := 'TRegEx(comp): LinePairedSeparator must countain two different chars or no chars at all';

    reeRegRepeatCalledInappropriately: Result := 'TRegEx(exec): RegRepeat Called Inappropriately';
    reeMatchPrimMemoryCorruption: Result := 'TRegEx(exec): MatchPrim Memory Corruption';
    reeMatchPrimCorruptedPointers: Result := 'TRegEx(exec): MatchPrim Corrupted Pointers';
    reeNoExpression: Result := 'TRegEx(exec): Not Assigned Expression Property';
    reeCorruptedProgram: Result := 'TRegEx(exec): Corrupted Program';
    reeNoInpitStringSpecified: Result := 'TRegEx(exec): No Input String Specified';
    reeOffsetMustBeGreaterThen0: Result := 'TRegEx(exec): Offset Must Be Greater Then 0';
    reeExecNextWithoutExec: Result := 'TRegEx(exec): ExecNext Without Exec[Pos]';
    reeGetInputStringWithoutInputString: Result := 'TRegEx(exec): GetInputString Without InputString';
    reeDumpCorruptedOpcode: Result := 'TRegEx(dump): Corrupted Opcode';
    reeLoopStackExceeded: Result := 'TRegEx(exec): Loop Stack Exceeded';
    reeLoopWithoutEntry: Result := 'TRegEx(exec): Loop Without LoopEntry !';

    reeBadPCodeImported: Result := 'TRegEx(misc): Bad p-code imported';
  else Result := 'Unknown error';
  end;
end; { of procedure TRegEx.Error
--------------------------------------------------------------}

function TRegEx.LastError: integer;
begin
  Result := fLastError;
  fLastError := reeOk;
end; { of function TRegEx.LastError
--------------------------------------------------------------}
...
class function TRegEx.VersionMajor: integer; //###0.944
begin
  Result := TRegExVersionMajor;
end; { of class function TRegEx.VersionMajor
--------------------------------------------------------------}

class function TRegEx.VersionMinor: integer; //###0.944
begin
  Result := TRegExVersionMinor;
end; { of class function TRegEx.VersionMinor
--------------------------------------------------------------}
...
end; { of constructor TRegEx.Create
--------------------------------------------------------------}
...
end; { of destructor TRegEx.Destroy
--------------------------------------------------------------}

class function TRegEx.InvertCaseFunction(const Ch: REChar): REChar;
begin
{$IFDEF UniCode}
  if Ch >= #128 then Result := Ch
  else
{$ENDIF}begin
...
end; { of function TRegEx.InvertCaseFunction
--------------------------------------------------------------}

function TRegEx.GetExpression: RegExprString;
begin
  if fExpression <> nil then Result := fExpression
  else Result := '';
end; { of function TRegEx.GetExpression
--------------------------------------------------------------}

procedure TRegEx.SetExpression(const s: RegExprString);
var
  Len: integer; //###0.950
begin
...
end; { of procedure TRegEx.SetExpression
--------------------------------------------------------------}

function TRegEx.GetSubExprMatchCount: integer;
begin
...
end; { of function TRegEx.GetSubExprMatchCount
--------------------------------------------------------------}

function TRegEx.GetMatchPos(Idx: integer): integer;
begin
...
end; { of function TRegEx.GetMatchPos
--------------------------------------------------------------}

function TRegEx.GetMatchLen(Idx: integer): integer;
begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned(fInputString)
    and Assigned(startp[Idx]) and Assigned(endp[Idx]) then begin
...
end; { of function TRegEx.GetMatchLen
--------------------------------------------------------------}

function TRegEx.GetMatch(Idx: integer): RegExprString;
begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned(fInputString)
    and Assigned(startp[Idx]) and Assigned(endp[Idx])
    {//then Result := copy (fInputString, MatchPos [Idx], MatchLen [Idx]) //###0.929}then SetString(Result, startp[idx], endp[idx] - startp[idx])
  else Result := '';
end; { of function TRegEx.GetMatch
--------------------------------------------------------------}

function TRegEx.GetModifierStr: RegExprString;
begin
  Result := '-';
  if ModifierI then Result := 'i' + Result
  else Result := Result + 'i';
  if ModifierR then Result := 'r' + Result
  else Result := Result + 'r';
  if ModifierS then Result := 's' + Result
  else Result := Result + 's';
  if ModifierG then Result := 'g' + Result
  else Result := Result + 'g';
  if ModifierM then Result := 'm' + Result
  else Result := Result + 'm';
  if ModifierX then Result := 'x' + Result
  else Result := Result + 'x';
  if Result[length(Result)] = '-' {// remove '-' if all modifiers are 'On'} then System.Delete(Result, length(Result), 1);
end; { of function TRegEx.GetModifierStr
--------------------------------------------------------------}

class function TRegEx.ParseModifiersStr(const AModifiers: RegExprString;
...
begin
  ...
  for i := 1 to length(AModifiers) do
    if AModifiers[i] = '-' then IsOn := false
    else begin
      if Pos(AModifiers[i], 'iI') > 0 then Mask := MaskModI
      else if Pos(AModifiers[i], 'rR') > 0 then Mask := MaskModR
      else if Pos(AModifiers[i], 'sS') > 0 then Mask := MaskModS
      else if Pos(AModifiers[i], 'gG') > 0 then Mask := MaskModG
      else if Pos(AModifiers[i], 'mM') > 0 then Mask := MaskModM
      else if Pos(AModifiers[i], 'xX') > 0 then Mask := MaskModX
      else begin
        Result := false;
        EXIT;
      end;
      if IsOn then AModifiersInt := AModifiersInt or Mask
      else AModifiersInt := AModifiersInt and not Mask;
    end;
end; { of function TRegEx.ParseModifiersStr
--------------------------------------------------------------}

procedure TRegEx.SetModifierStr(const AModifiers: RegExprString);
begin
  if not ParseModifiersStr(AModifiers, fModifiers) then Error(reeModifierUnsupported);
end; { of procedure TRegEx.SetModifierStr
--------------------------------------------------------------}

function TRegEx.GetModifier(AIndex: integer): boolean;
var
  Mask: integer;
begin
  Result := false;
...
end; { of function TRegEx.GetModifier
--------------------------------------------------------------}

procedure TRegEx.SetModifier(AIndex: integer; ASet: boolean);
var
  Mask: integer;
begin
  case AIndex of
  ...
  if ASet then fModifiers := fModifiers or Mask
  else fModifiers := fModifiers and not Mask;
end; { of procedure TRegEx.SetModifier
--------------------------------------------------------------}
...
procedure TRegEx.InvalidateProgramm;
begin
  if programm <> nil then begin
    FreeMem(programm);
    programm := nil;
  end;
end; { of procedure TRegEx.InvalidateProgramm
--------------------------------------------------------------}

procedure TRegEx.Compile; //###0.941
begin
...
end; { of procedure TRegEx.Compile
--------------------------------------------------------------}

function TRegEx.IsProgrammOk: boolean;
{$IFNDEF UniCode}
var
  i: integer;
{$ENDIF}
begin
  ...
  // check modifiers
  if fModifiers <> fProgModifiers then InvalidateProgramm; //###0.941
...
{$ENDIF}
  ...
  if programm = nil then EXIT // error was set/raised by Compile (was reeExecAfterCompErr)
  else if programm[0] <> MAGIC {// Program corrupted.} then Error(reeCorruptedProgram)
  else Result := true;
end; { of function TRegEx.IsProgrammOk
--------------------------------------------------------------}

procedure TRegEx.Tail(p: PRegExprChar; val: PRegExprChar);
// set the next-pointer at the end of a node chain
var
  scan: PRegExprChar;
  temp: PRegExprChar;
...
begin
  ...
  until false;
  // Set Next 'pointer'
  if val < scan then PRENextOff(scan + REOpSz)^ := -(scan - val) //###0.948
...
end; { of procedure TRegEx.Tail
--------------------------------------------------------------}

procedure TRegEx.OpTail(p: PRegExprChar; val: PRegExprChar);
...
end; { of procedure TRegEx.OpTail
--------------------------------------------------------------}

function TRegEx.EmitNode(op: TREOp): PRegExprChar; //###0.933
...
end; { of function TRegEx.EmitNode
--------------------------------------------------------------}

procedure TRegEx.EmitC(b: REChar);
// emit a byte to code
begin
...
end; { of procedure TRegEx.EmitC
--------------------------------------------------------------}

procedure TRegEx.InsertOperator(op: TREOp; opnd: PRegExprChar; sz: integer);
// insert an operator in front of already-emitted operand
// Means relocating the operand.
var
...
end; { of procedure TRegEx.InsertOperator
--------------------------------------------------------------}
...
function TRegEx.CompileRegExpr(exp: PRegExprChar): boolean;
// compile a regular expression into internal code
...
{$ENDIF}
    ...
    if PREOp(regnext(scan))^ = EEND then begin // Only one top-level choice.
      ...
      // Starting-point info.
      if PREOp(scan)^ = EXACTLY then regstart := (scan + REOpSz + RENextOffSz)^
      else if PREOp(scan)^ = BOL then inc(reganch);
...
end; { of function TRegEx.CompileRegExpr
--------------------------------------------------------------}

function TRegEx.ParseReg(paren: integer; var flagp: integer): PRegExprChar;
// regular expression, i.e. main body or parenthesized thing
...
begin
  ...
  if ret <> nil then Tail(ret, br) // OPEN -> first.
  else ret := br;
  ...
  if paren <> 0 then ender := EmitNode(TREOp(ord(CLOSE) + parno))
  ...
  if (paren = 0) and (regparse^ <> #0) then begin
    if regparse^ = ')' then Error(reeCompParseRegUnmatchedBrackets2)
    else Error(reeCompParseRegJunkOnEnd);
    EXIT;
  end;
  fCompModifiers := SavedModifiers; // restore modifiers of parent
  Result := ret;
end; { of function TRegEx.ParseReg
--------------------------------------------------------------}

function TRegEx.ParseBranch(var flagp: integer): PRegExprChar;
// one alternative of an | operator
...
begin
  ...
  while (regparse^ <> #0) and (regparse^ <> '|')
    ...
    if chain = nil {// First piece.} then flagp := flagp or flags and SPSTART
    else Tail(chain, latest);
    chain := latest;
  end;
  if chain = nil then EmitNode(NOTHING); // Loop ran zero times.

  Result := ret;
end; { of function TRegEx.ParseBranch
--------------------------------------------------------------}

function TRegEx.ParsePiece(var flagp: integer): PRegExprChar;
// something followed by possible [*+?{]
...
{$ELSE}
    if ANonGreedyOp then TheOp := LOOPNG
    else TheOp := LOOP;
...
{$ENDIF}
  ...
  procedure EmitSimpleBraces(ABracesMin, ABracesMax: TREBracesArg;
    ANonGreedyOp: boolean); //###0.940
  begin
    if ANonGreedyOp {//###0.940} then TheOp := BRACESNG
    else TheOp := BRACES;
  ...
  case op of
    '*': begin
        ...
        if (flags and SIMPLE) = 0 then begin
          if NonGreedyOp {//###0.940} then EmitComplexBraces(0, MaxBracesArg, NonGreedyOp)
          ...
          if NonGreedyOp {//###0.940} then TheOp := STARNG
          else TheOp := STAR;
          InsertOperator(TheOp, Result, REOpSz + RENextOffSz);
        end;
        if NonGreedyCh then inc(regparse); //###0.940
        // Skip extra char ('?')
      end; { of case '*'}
    '+': begin
        ...
        NonGreedyOp := NonGreedyCh or ((fCompModifiers and MaskModG) = 0); //###0.940
        if (flags and SIMPLE) = 0 then begin
          if NonGreedyOp {//###0.940} then EmitComplexBraces(1, MaxBracesArg, NonGreedyOp)
          else begin // Emit x+ as x(&|), where & means "self".
          ...
          if NonGreedyOp {//###0.940} then TheOp := PLUSNG
          else TheOp := PLUS;
          InsertOperator(TheOp, Result, REOpSz + RENextOffSz);
        end;
        if NonGreedyCh then inc(regparse); //###0.940
    ...
    '?': begin
        ...
        if NonGreedyOp then begin //###0.940  // We emit x?? as x{0,1}?
          if (flags and SIMPLE) = 0 then EmitComplexBraces(0, 1, NonGreedyOp)
    ...
    '{': begin
        ...
        inc(regparse);
        p := regparse;
        while Pos(regparse^, '0123456789') > 0 do inc(regparse); // <min> MUST appear
        ...
        if regparse^ = ',' then begin
          ...
          if p = regparse then BracesMax := MaxBracesArg
        ...
        NonGreedyCh := (regparse + 1)^ = '?'; //###0.940
        NonGreedyOp := NonGreedyCh or ((fCompModifiers and MaskModG) = 0); //###0.940
        if (flags and SIMPLE) <> 0 then EmitSimpleBraces(BracesMin, BracesMax, NonGreedyOp)
...
end; { of function TRegEx.ParsePiece
--------------------------------------------------------------}

function TRegEx.ParseAtom(var flagp: integer): PRegExprChar;
// the lowest level
...
{$ENDIF}
  ...
  begin
    if (fCompModifiers and MaskModI) <> 0 then ret := EmitNode(EXACTLYCI)
...
{$IFDEF UseAsserts}
      Assert(RangeLen > 0, 'TRegEx.ParseAtom(subroutine EmitRangeC): empty range'); // impossible, but who knows..
      Assert(RangeChMin <= RangeChMax, 'TRegEx.ParseAtom(subroutine EmitRangeC): RangeChMin > RangeChMax'); // impossible, but who knows..
{$ENDIF}
...
begin
  ...
  case (regparse - 1)^ of
    '^': if ((fCompModifiers and MaskModM) = 0)
      or ((fLineSeparators = '') and not fLinePairedSeparatorAssigned) then ret := EmitNode(BOL)
      else ret := EmitNode(BOLML);
    '$': if ((fCompModifiers and MaskModM) = 0) or ((fLineSeparators = '') and
        not fLinePairedSeparatorAssigned) then ret := EmitNode(EOL)
    ...
    '[': begin
        if regparse^ = '^' then begin // Complement of range.
          if (fCompModifiers and MaskModI) <> 0 then ret := EmitRange(ANYBUTCI)
          else ret := EmitRange(ANYBUT);
          inc(regparse);
        end
        else
          if (fCompModifiers and MaskModI) <> 0 then ret := EmitRange(ANYOFCI)
...
{$ENDIF}
          ...
          '1'..'9': begin //###0.936
              if (fCompModifiers and MaskModI) <> 0 then ret := EmitNode(BSUBEXPCI)
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF})) then begin //###0.941 \x
        if regparse^ = '#' then begin // Skip eXtended comment
          ...
          while (regparse^ = #$D) or (regparse^ = #$A) do inc(regparse); // skip comment terminator
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF} do inc(regparse);
        ...
        ender := (regparse + len)^;
        if (len > 1) and ((ender = '*') or (ender = '+') or (ender = '?') or (ender = '{')) then dec(len); // Back off clear of ?+*{ operand.
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF}) then EmitC(regparse^);
  ...
  Result := ret;
end; { of function TRegEx.ParseAtom
--------------------------------------------------------------}

function TRegEx.GetCompilerErrorPos: integer;
begin
  Result := 0;
  if (regexpbeg = nil) or (regparse = nil) then EXIT; // not in compiling mode ?
  Result := regparse - regexpbeg;
end; { of function TRegEx.GetCompilerErrorPos
--------------------------------------------------------------}
...
{$IFNDEF UseSetOfChar}
function TRegEx.StrScanCI(s: PRegExprChar; ch: REChar): PRegExprChar; //###0.928 - now method of TRegEx
begin
  while (s^ <> #0) and (s^ <> ch) and (s^ <> InvertCase(ch)) do inc(s);
  if s^ <> #0 then Result := s
  else Result := nil;
end; { of function TRegEx.StrScanCI
--------------------------------------------------------------}
{$ENDIF}

function TRegEx.regrepeat(p: PRegExprChar; AMax: integer): integer;
// repeatedly match something simple, report how many
var
  scan: PRegExprChar;
  opnd: PRegExprChar;
...
{$IFNDEF UseSetOfChar} //###0.929
    ANYLETTER:
      while (Result < TheMax) and
        (Pos(scan^, fWordChars) > 0) do begin //###0.940
...
{$ENDIF}
  ...
  reginput := scan;
end; { of function TRegEx.regrepeat
--------------------------------------------------------------}

function TRegEx.regnext(p: PRegExprChar): PRegExprChar;
// dig the "next" pointer out of a node
var
  offset: TRENextOff;
begin
  ...
  if offset = 0 then Result := nil
  else Result := p + offset;
end; { of function TRegEx.regnext
--------------------------------------------------------------}

function TRegEx.MatchPrim(prog: PRegExprChar): boolean;
// recursively matching routine
    ...
    case scan^ of
      ...
      BOLML: if reginput > fInputStart then begin
          ...
          if (nextch <> fLinePairedSeparatorTail)
            ...
            or ((reginput - 2)^ <> fLinePairedSeparatorHead) then begin
...
{$ENDIF} then EXIT;
      ...
      EOLML: if reginput^ <> #0 then begin
          nextch := reginput^;
          if (nextch <> fLinePairedSeparatorHead)
            or ((reginput + 1)^ <> fLinePairedSeparatorTail) then begin
            if (nextch = fLinePairedSeparatorTail)
...
{$IFNDEF UseSetOfChar} //###0.929
      ANYLETTER: begin
          if (reginput^ = #0) or (Pos(reginput^, fWordChars) <= 0) then EXIT; //###0.943
      ...
      ANYSPACE: begin
          if (reginput^ = #0) or not (Pos(reginput^, fSpaceChars) > 0) then EXIT; //###0.943
          inc(reginput);
        end;
      NOTSPACE: begin
          if (reginput^ = #0) or (Pos(reginput^, fSpaceChars) > 0) then EXIT; //###0.943
          inc(reginput);
        end;
{$ENDIF}
      EXACTLYCI: begin
...
{$ENDIF}
      ...
      Succ(OPEN)..TREOp(Ord(OPEN) + NSUBEXP - 1): begin //###0.929
          ...
          if not Result then startp[no] := save; //###0.936
      ...
      Succ(CLOSE)..TREOp(Ord(CLOSE) + NSUBEXP - 1): begin //###0.929
          ...
          Result := MatchPrim(next);
          if not Result then endp[no] := save; //###0.936
      ...
      BRANCH: begin
          if (next^ <> BRANCH) {// No choice.} then next := scan + REOpSz + RENextOffSz // Avoid recursion
          else begin
            repeat
              save := reginput;
...
{$ENDIF}
      STAR, PLUS, BRACES, STARNG, PLUSNG, BRACESNG: begin
          ...
          if (scan^ = STAR) or (scan^ = STARNG) then BracesMin := 0 // STAR
          else if (scan^ = PLUS) or (scan^ = PLUSNG) then BracesMin := 1 // PLUS
          else begin // BRACES
            BracesMin := PREBracesArg(scan + REOpSz + RENextOffSz)^;
            BracesMax := PREBracesArg(scan + REOpSz + RENextOffSz + REBracesArgSz)^;
          end;
...
end; { of function TRegEx.MatchPrim
--------------------------------------------------------------}

{$IFDEF UseFirstCharSet} //###0.929
procedure TRegEx.FillFirstCharSet(prog: PRegExprChar);
var
  scan: PRegExprChar; // Current node.
  next: PRegExprChar; // Next node.
  opnd: PRegExprChar;
...
begin
  ...
  while scan <> nil do begin
    ...
    case PREOp(scan)^ of
      ...
      BRANCH: begin
          if (PREOp(next)^ <> BRANCH) {// No choice.} then next := scan + REOpSz + RENextOffSz // Avoid recursion.
          else begin
            repeat
...
function TRegEx.Exec(const AInputString: RegExprString): boolean;
begin
  InputString := AInputString;
  Result := ExecPrim(1);
end; { of function TRegEx.Exec
--------------------------------------------------------------}

{$IFDEF OverMeth}
{$IFNDEF FPC}
function TRegEx.Exec: boolean;
begin
  Result := ExecPrim(1);
end; { of function TRegEx.Exec
--------------------------------------------------------------}
{$ENDIF}
function TRegEx.Exec(AOffset: integer): boolean;
begin
...
function TRegEx.ExecPos(AOffset: integer{$IFDEF DefParam} = 1{$ENDIF}): boolean;
begin
  Result := ExecPrim(AOffset);
end; { of function TRegEx.ExecPos
--------------------------------------------------------------}

function TRegEx.ExecPrim(AOffset: integer): boolean;
...
begin
  ...
  if not IsProgrammOk then EXIT; //###0.929
  ...
  if AOffset > (InputLen + 1) then EXIT; // for matching empty string after last char.
  ...
  if regmust <> nil then begin
    ...
    until s = nil;
    if s = nil then EXIT; // Not present.
  end;
...
{$ELSE}
      Result := RegMatch(s);
{$ENDIF}
      if Result or (s^ = #0) then EXIT // Exit on a match or after testing the end-of-string.
      else ClearMatchs; //###0.949
      inc(s);
    until false;
  ...
  // Failure
end; { of function TRegEx.ExecPrim
--------------------------------------------------------------}

function TRegEx.ExecNext: boolean;
var
  offset: integer;
begin
  Result := false;
  ...
  if endp[0] = startp[0] then inc(Offset); //###0.929
  // prevent infinite looping if empty string match r.e.
  Result := ExecPrim(Offset);
end; { of function TRegEx.ExecNext
--------------------------------------------------------------}

function TRegEx.GetInputString: RegExprString;
begin
  if not Assigned(fInputString) then begin
  ...
  Result := fInputString;
end; { of function TRegEx.GetInputString
--------------------------------------------------------------}

procedure TRegEx.SetInputString(const AInputString: RegExprString);
var
  Len: integer;
  i: integer;
begin
...
end; { of procedure TRegEx.SetInputString
--------------------------------------------------------------}

procedure TRegEx.SetLineSeparators(const AStr: RegExprString);
begin
  if AStr <> fLineSeparators then begin
    fLineSeparators := AStr;
    InvalidateProgramm;
  end;
end; { of procedure TRegEx.SetLineSeparators
--------------------------------------------------------------}

procedure TRegEx.SetLinePairedSeparator(const AStr: RegExprString);
begin
...
end; { of procedure TRegEx.SetLinePairedSeparator
--------------------------------------------------------------}

function TRegEx.GetLinePairedSeparator: RegExprString;
begin
  if fLinePairedSeparatorAssigned then begin
{$IFDEF UniCode}
...
end; { of function TRegEx.GetLinePairedSeparator
--------------------------------------------------------------}

function TRegEx.Substitute(const ATemplate: RegExprString): RegExprString;
...
var
  ...
  begin
    ...
    if (p < TemplateEnd) and (p^ = '&') then inc(p) // this is '$&' or '${&}'
    else
...
{$ENDIF} do begin
    ...
    if Delimited then
      if (p < TemplateEnd) and (p^ = '}') then inc(p) // skip right curly brace
...
begin
  ...
  while p < TemplateEnd do begin
    Ch := p^;
    inc(p);
    if Ch = '$' then n := ParseVarName(p)
    else n := -1;
    if n >= 0 then begin
      if (n < NSUBEXP) and Assigned(startp[n]) and Assigned(endp[n]) then inc(ResultLen, endp[n] - startp[n]);
    end
    else begin
      if (Ch = EscChar) and (p < TemplateEnd) then inc(p); // quoted or special char followed
      inc(ResultLen);
    end;
  end;
...
end; { of function TRegEx.Substitute
--------------------------------------------------------------}

procedure TRegEx.Split(AInputStr: RegExprString; APieces: TStrings);
var
  PrevPos: integer;
begin
  PrevPos := 1;
...
end; { of procedure TRegEx.Split
--------------------------------------------------------------}

function TRegEx.Replace(AInputStr: RegExprString; const AReplaceStr: RegExprString;
  AUseSubstitution: boolean{$IFDEF DefParam} = False{$ENDIF}): RegExprString;
var
  PrevPos: integer;
begin
  ...
  if Exec(AInputStr) then
    repeat
      Result := Result + System.Copy(AInputStr, PrevPos,
        MatchPos[0] - PrevPos);
      if AUseSubstitution {//###0.946} then Result := Result + Substitute(AReplaceStr)
      else Result := Result + AReplaceStr;
      PrevPos := MatchPos[0] + MatchLen[0];
    until not ExecNext;
  Result := Result + System.Copy(AInputStr, PrevPos, MaxInt); // Tail
end; { of function TRegEx.Replace
--------------------------------------------------------------}

function TRegEx.ReplaceEx(AInputStr: RegExprString;
  AReplaceFunc: TRegExReplaceFunction)
  : RegExprString;
var
  PrevPos: integer;
begin
  ...
  if Exec(AInputStr) then
    ...
    until not ExecNext;
  Result := Result + System.Copy(AInputStr, PrevPos, MaxInt); // Tail
end; { of function TRegEx.ReplaceEx
--------------------------------------------------------------}

{$IFDEF OverMeth}
function TRegEx.Replace(AInputStr: RegExprString;
  AReplaceFunc: TRegExReplaceFunction)
  : RegExprString;
begin
...
{$IFDEF RegExpPCodeDump}
function TRegEx.DumpOp(op: TREOp): RegExprString;
...
{$ENDIF}
  ...
  Result := ':' + Result;
end; { of function TRegEx.DumpOp
--------------------------------------------------------------}

function TRegEx.Dump: RegExprString;
...
begin
  ...
  while op <> EEND do begin // While that wasn't END last time...
    ...
    next := regnext(s);
    if next = nil then Result := Result + ' (0)' // Next ptr.

    else begin
      if next > s then Diff := next - s //###0.948 PWideChar subtraction workaround (see comments in Tail method for details)
    ...
    if op = ANYOFFULLSET then begin
      for Ch := #0 to #255 do
        if Ch in PSetOfREChar(s)^ then
          if Ch < ' ' then Result := Result + '#' + IntToStr(Ord(Ch)) //###0.936
...
{$IFDEF UseFirstCharSet} //###0.929
  ...
  for Ch := #0 to #255 do
    if Ch in FirstCharSet then begin
      if Ch < ' ' then Result := Result + '#' + IntToStr(Ord(Ch)) //###0.948
      else Result := Result + Ch;
    end;
{$ENDIF}
  Result := Result + #$D#$A;
end; { of function TRegEx.Dump
--------------------------------------------------------------}
{$ENDIF}
...
{$ENDIF}
procedure TRegEx.Error(AErrorID: integer);
{$IFDEF reRealExceptionAddr}
...
begin
  ...
  if AErrorID < 1000 {// compilation error ?} then e := ERegExpr.Create(ErrorMsg(AErrorID) // yes - show error pos
...
{$IFDEF FPC}
initialization
  RegExprInvertCaseFunction := TRegEx.InvertCaseFunction;
{$ENDIF}
end.
